...
abstract class TwoInputGate extends LogicGate {
    ...
    abstract public boolean newOutputDetected(float time);
...
class AndGate extends TwoInputGate {
    ...
    public boolean newOutputDetected( float time ) {
	boolean initialValue = value;
	updateValue();
	if (initialValue != value) {
	    System.out.println( "At " + time + delay + " " + toString() 
...
class NotGate extends LogicGate {
	...
	Simulator.schedule( delay,
	    (float t) -> inputChangeEvent( t, 1, false )
	);
    }
    ...
    boolean newOutputDetected(float time, boolean v) {
	if (v == value) {
	    updateValue();
	    System.out.println( "At " + time + delay + " " + toString() 
	        + " out " + " changes to " + value
